...
if (isset($_GET["NXB_Ma"])) {
    ...
    $checkSql = "SELECT COUNT(*) AS book_count FROM sach WHERE NXB_Ma = '$NXB_Ma'";
    ...
    if ($checkRow['book_count'] > 0) {
        ...
        $sql = "DELETE FROM nhaxuatban WHERE NXB_Ma = '$NXB_Ma'";
    ...
    document.addEventListener("DOMContentLoaded", function() {
        <?php if ($deleteSuccess): ?> // Check if deletion was successful
            swal({
                ...
                window.location.href = "../../nhaxuatban_all.php"; // Redirect after closing alert
            });
        <?php elseif ($hasBooks): ?> // Check if there are associated books
            swal({
                title: "Không thể xóa!",
                text: "Nhà xuất bản này đang được sử dụng trong dữ liệu sách, không thể xóa!",
                icon: "error",
                button: "OK",
            }).then(() => {
                window.location.href = "../../nhaxuatban_all.php"; // Redirect after closing alert
